...
import ResultItem from './components/ResultTab'
...
class App extends Component {
  state = {
    randomId: imagesList[0].id,
    score: 0,
    thumbListId: tabsList[0].tabId,
    sec: 60,
    result: false,
  }
  ...
  tick = () => {
    ...
    if (sec > 0) {
      this.setState(preState => ({sec: preState.sec - 1}))
    } else {
  ...
  onCheckImageId = id => {
    ...
    if (randomId === id) {
      this.setState(preState => ({
        ...
        randomId: imagesList[Math.floor(Math.random() * imagesList.length)].id,
      }))
    } else {
      this.setState({
        randomId: imagesList[Math.floor(Math.random() * imagesList.length)].id,
  ...
  onChangeTab = tabId => {
    this.setState({thumbListId: tabId})
  }

  render() {
    const {randomId, score, thumbListId, sec, result} = this.state

    const randomList = imagesList.filter(eachItem => eachItem.id === randomId)
    const sortedThumbList = imagesList.filter(
      each => each.category === thumbListId,
    )
    const heading = result ? 'Scorecard' : 'Match Game'
    return (
      <div>
        <h1 className="gameHeading">{heading}</h1>
        <nav>
          <ul className="navBar">
            <li>
              <img
            ...
            <li className="scoreBoard">
              <p>
                Score: <span className="counterTimer">{score}</span>
              </p>
              <img
                className="timer"
                ...
                alt="timer"
              />
              <p className="counterTimer">{sec} sec</p>
            </li>
          </ul>
        </nav>
        <div className="container">
          {result ? (
            ...
            <>
              <img
                className="appImage"
                src={randomList[0].imageUrl}
                alt="match"
              />
              <ul className="tabsListContainer">
                {tabsList.map(eachList => (
                  <TabItem
                    isClicked={eachList.tabId === thumbListId}
              ...
              <ul className="thumbImageContainer">
                {sortedThumbList.map(eachItem => (
                  <ImageItem
